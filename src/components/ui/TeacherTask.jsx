...
const TeacherTask = ({ title, taskText, pin }) => {
  ...
  if (!isUnlocked) {
    return (
      <div className="relative mt-10 sm:mt-16 md:mt-24 rounded-2xl sm:rounded-3xl p-[1px] overflow-hidden group">
        <div className="absolute inset-0 bg-gradient-to-r from-rose-400 via-pink-400 to-purple-400 opacity-40"></div>
        <div className="relative bg-white/90 backdrop-blur-xl p-5 sm:p-8 md:p-12 rounded-2xl sm:rounded-3xl h-full flex flex-col items-center shadow-xl text-center transition-all">
          ...
          <h3 className="text-xl sm:text-2xl font-bold text-slate-800 mb-6 sm:mb-8">{title}</h3>
          ...
          <form onSubmit={handleUnlock} className="flex flex-col items-center w-full max-w-sm">
            <div className="relative w-full mb-4">
              ...
              <input 
                ...
                className={`w-full pl-12 pr-4 py-3 min-h-[48px] rounded-xl border ${error ? 'border-rose-400 bg-rose-50' : 'border-slate-300 bg-white'} focus:outline-none focus:ring-2 ${error ? 'focus:ring-rose-400' : 'focus:ring-purple-400'} transition-all text-center tracking-widest text-lg`}
                value={enteredPin}
            ...
            <button 
              type="submit"
              className="w-full bg-slate-900 hover:bg-slate-800 text-white font-medium py-3 min-h-[48px] rounded-xl transition-colors flex items-center justify-center gap-2 cursor-pointer"
            >
  ...
  return (
    <div className="relative mt-10 sm:mt-16 md:mt-24 rounded-2xl sm:rounded-3xl p-[1px] overflow-hidden group">
      ...
      <div className="relative bg-white/90 backdrop-blur-xl p-5 sm:p-8 md:p-10 rounded-2xl sm:rounded-3xl h-full flex flex-col shadow-xl">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 sm:mb-8 gap-4 sm:gap-6">
          <div className="flex items-center gap-4 sm:gap-5">
            <div className="p-3 bg-blue-100 rounded-2xl text-blue-600 border border-blue-200 shadow-[0_0_15px_rgba(59,130,246,0.3)] shrink-0">
          ...
          <button
            onClick={handleCopy}
            className={`w-full md:w-auto justify-center flex items-center gap-2 px-5 py-2.5 min-h-[44px] rounded-xl font-medium text-sm transition-all duration-300 cursor-pointer ${
              copied 
        ...
        <div className="bg-slate-50 p-4 sm:p-6 rounded-2xl border border-slate-200 text-slate-700 whitespace-pre-wrap leading-relaxed font-mono text-xs sm:text-sm overflow-x-auto break-words">
          {taskText}
        </div>
      </div>
    </div>
