...
export default function ExamResult() {
  ...
  const fetchResults = async () => {
    ...
    if (data && data.length > 0) {
      setResults(data);
      setSessionTitle(data[0].exam_sessions?.title || 'ผลการสอบ');
      setTotalScore(data[0].exam_sessions?.total_score || data[0].total_questions || 10);
    }
    setLoading(false);
  };
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4 relative overflow-hidden">
      ...
      <div className="max-w-md w-full bg-white/90 backdrop-blur-xl rounded-3xl shadow-2xl border border-white/60 p-8 text-center relative z-10 space-y-6">
        ...
        <div>
          ...
          <p className="text-xs text-zinc-500 mt-1">{sessionTitle}</p>
